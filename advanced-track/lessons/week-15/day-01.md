...
本周主题：MCP Server：把你的工具标准化暴露给 AI
...
- [ ] 用一句话写清楚真实问题：MCP 不是另一个模型，而是把工具和资源用统一协议提供给 AI 客户端。它适合封装公司内部查询能力。
- [ ] 画出前端、服务端、模型 / 数据、验证层之间的边界。
- [ ] 写清楚前端切入点：前端仍然通过你的应用使用能力，但你要理解 MCP Server 在架构中位于工具服务层。
- [ ] 列出本周必须补的后端基础：MCP Tool、MCP Resource、JSON Schema、最小权限暴露、本地调试。
- [ ] 列出本周不做什么，避免范围失控。
...
`advanced-track/lessons/week-15/day-01.md`

周总结写入：

`advanced-track/reviews/week-15.md`
