...
- [ ] 用一句话写清楚真实问题：一旦工具有写操作，风险就完全不同。删除、修改、发送、创建都必须有确认、幂等和审计。
- [ ] 画出前端、服务端、模型 / 数据、验证层之间的边界。
- [ ] 写清楚前端切入点：前端必须出现二次确认面板：展示将要执行的动作、参数、影响范围和取消按钮。
- [ ] 列出本周必须补的后端基础：写操作权限、幂等键、审计日志、二次确认 token、副作用隔离。
- [ ] 列出本周不做什么，避免范围失控。
...
`advanced-track/lessons/week-13/day-01.md`
