...
- [ ] 用一句话写清楚真实问题：AI 不是普通接口，用户可能等 10 秒才看到完整结果。前端要处理首字延迟、流式输出、中途取消、重试和失败恢复。
- [ ] 画出前端、服务端、模型 / 数据、验证层之间的边界。
- [ ] 写清楚前端切入点：你重点练的是状态机和交互体验：按钮什么时候禁用、取消后输入是否保留、失败后是否能重试。
- [ ] 列出本周必须补的后端基础：SSE 响应头、ReadableStream 基础、AbortController、连接断开处理、首字延迟记录。
- [ ] 列出本周不做什么，避免范围失控。
...
`advanced-track/lessons/week-02/day-01.md`
