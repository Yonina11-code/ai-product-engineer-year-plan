...
- [ ] 用一句话写清楚真实问题：真实用户不会按文档原字段名提问，纯关键词和纯向量都会失败。你要学会比较召回效果。
- [ ] 画出前端、服务端、模型 / 数据、验证层之间的边界。
- [ ] 写清楚前端切入点：前端做一个检索调试台：输入问题后展示候选文档、分数、metadata 和被选中的上下文。
- [ ] 列出本周必须补的后端基础：embedding 调用、向量相似度、topK、关键词索引、召回结果合并。
- [ ] 列出本周不做什么，避免范围失控。
...
`advanced-track/lessons/week-08/day-01.md`
