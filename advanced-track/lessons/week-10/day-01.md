...
- [ ] 用一句话写清楚真实问题：RAG Demo 很容易看起来能用，但一换问题就失败。你要建立能反复跑的评测集和失败归因。
- [ ] 画出前端、服务端、模型 / 数据、验证层之间的边界。
- [ ] 写清楚前端切入点：做一个评测结果页或表格，能看到每条样本的问题、期望、实际、引用和失败类型。
- [ ] 列出本周必须补的后端基础：评测数据格式、批量运行脚本、指标计算、失败归因、回归对比。
- [ ] 列出本周不做什么，避免范围失控。
...
`advanced-track/lessons/week-10/day-01.md`
